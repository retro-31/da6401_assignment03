...
def main():
    ...
    wandb.init(project="assignment-03", config=defaults)
    cfg = wandb.config

    # ── 2) derive a run‐name from hyperparams ────────────────────────────
    run_name = (
        f"{cfg.cell_type}"
        f"_enc{cfg.encoder_layers}"
        f"_dec{cfg.decoder_layers}"
    ...
    # temp file for this run’s best checkpoint
    tmp_ckpt = f"runs/{run_name}_tmp.pt"
    # global best records
    global_json  = "runs/global_best.json"
    global_model = "runs/best_model.pt"
    ...
    for epoch in range(cfg.epochs):
        ...
        for src, tgt in train_loader:
            ...
            loss = criterion(outputs.view(-1, outputs.size(-1)), tgt[:,1:].reshape(-1))
            loss.backward()
            optimizer.step()
        ...
        # if this epoch is best so far for _this_ run, save temp checkpoint
    ...
    # if this run improves on global best, overwrite best_model.pt
    if run_best_acc > prev_acc:
        # update global record
        with open(global_json, "w") as f:
            json.dump({"best_acc": run_best_acc, "run_name": run_name}, f)
        # replace the global model file
        os.replace(tmp_ckpt, global_model)
        print(f"🏆 New global best: {run_name} @ {run_best_acc:.4f}")
    else:
        # discard this run’s checkpoint
        os.remove(tmp_ckpt)
